...
contract MegaSwapFund is Context, ERC165, IERC1363Receiver, IERC1363Spender, IMegaSwapFund {
    ...
    uint256 constant public THREAD_COUNT = 20;

    // Value of 1 token #0 expressed in token #1 tokens.
    uint256 constant public SWAP_RATE_1 = 100;

    // Minimum token #0 amount that can be received and swapped at thread #0.
    uint256 constant public MIN_SWAPPABLE_IN_AMOUNT_0 = 1e9;

    // Minimum token #1 amount that can be received and swapped at thread #0.
    uint256 constant public MIN_SWAPPABLE_IN_AMOUNT_1 = MIN_SWAPPABLE_IN_AMOUNT_0 * SWAP_RATE_1;
    ...
    constructor(
        ...
        TOKEN_0 = address(uint160(uint256(keccak256(abi.encodePacked(bytes1(0xd6), bytes1(0x94), address(this), bytes1(0x01))))));
        TOKEN_1 = address(uint160(uint256(keccak256(abi.encodePacked(bytes1(0xd6), bytes1(0x94), address(this), bytes1(0x02))))));
    ...
    function periodNumbers(uint256 fromThreadId, uint256 toThreadId) public view returns (uint256[] memory) {
        ...
        uint256 mlength;
        unchecked { mlength = toThreadId - fromThreadId + 1; }
    ...
    function totalSwapinCounters(uint256 fromThreadId, uint256 toThreadId, uint256 tokenId) public view returns (uint256[] memory) {
        require(fromThreadId < toThreadId && toThreadId < THREAD_COUNT, "MegaSwapFund: wrong thread ID(s)");
        require(tokenId < 2, "MegaSwapFund: wrong token ID");
    ...
    function getSwappableOutBalance(uint256 threadId, uint256 tokenId) public view returns (uint256) {
        ...
        uint256 _pairedAmount = MAX_SWAPIN_COUNTERS[pairedTokenId][threadId] - _reciprocalSwaps[threadId].swapins[pairedTokenId];
        if (_pairedAmount == 0) return 0;

        _pairedAmount = tokenId == 0
            ? _pairedAmount / SWAP_RATE_1
            : _pairedAmount * SWAP_RATE_1;
     ...
     */
    function onTransferReceived(address, address sender, uint256 amount, bytes calldata data) public override returns (bytes4) {
        uint256 tokenId;
        uint256 pairedTokenId;
        ...
        // Initiate finalization.
            ...
            if (amount > finalAmount) {
                ...
                unchecked { returnAmount = amount - finalAmount; }

                IERC20(tokenId == 0 ? TOKEN_0 : TOKEN_1).transfer(sender, returnAmount);
            }

            // Calculate paired token amount.
            uint256 pairedFinalAmount = tokenId == 0
                ? finalAmount * SWAP_RATE_1
                : finalAmount / SWAP_RATE_1;
    ...
    function _getPairFinalizationAmount(uint256 threadId, uint256 tokenId, uint256 pairedTokenId) private view returns (uint256) {
    ...
    function _onReceivedWithoutData(address sender, uint256 tokenId, uint256 pairedTokenId, uint256 amount) private {
        ...
        // Calculate paired token amount.
        uint256 pairedAmount = tokenId == 0
            ? amount * SWAP_RATE_1
            : amount / SWAP_RATE_1;
